 ...
 */
@Path("invalidate-conf")
public class InvalidateConfigurationResource {
    
    final static Logger LOGGER = Logger.getLogger(InvalidateConfigurationResource.class);

    @GET
    @Produces("application/json")
    public Response invalidate() {
        
        LOGGER.info("Iniciando servicio de Invalidación de configuracion");
        
        PropertiesConfiguration.invalidate();
        ...
        LOGGER.debug("JSON RESPONSE - INVALIDATE = " + jsonBody);
        
        ResponseBuilder responseBuilder = Response.ok(jsonBody, MediaType.APPLICATION_JSON);
        Response response = responseBuilder.build();
        
        LOGGER.info("Finalizado servicio de Invalidación de configuración");
        return response;
    }
